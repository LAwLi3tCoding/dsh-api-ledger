...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function RouteSettings(props) {
      ...
      var price = props.settings.pricing[route.route + '/' + model] || { mode: 'none' }
      ...
      function selectModel(value) {
        modelState[1](value)
        var p = props.settings.pricing[route.route + '/' + value] || { mode: 'none' }
        ...
        React.createElement('form', { onSubmit: save },
          React.createElement('div', { className: 'api-ledger-config' },
            ...
            React.createElement('label', null, t('settings.mode'), React.createElement('select', { value: form.mode, disabled: !model, onChange: function (e) { set('mode', e.target.value) } },
              ['none', 'reference', 'custom'].map(function (mode) { return React.createElement('option', { key: mode, value: mode, disabled: mode === 'reference' && props.settings.referenceModels.indexOf(model) < 0 }, t({ none: 'settings.none', reference: 'settings.reference', custom: 'settings.custom' }[mode])) }))),
          ...
          React.createElement('p', { style: S.faintText }, t(form.mode === 'reference' ? 'settings.referenceNote' : form.mode === 'none' ? 'settings.none' : 'settings.unit')),
    ...
    function RecentCalls(props) {
      ...
      var rows = props.rows.map(function (row, i) {
          ...
          return React.createElement('li', { key: i },
            React.createElement('div', null, row.model || '—', React.createElement('small', null, when + ' · ' + (row.route || '—') + ' · ' + (props.labels[row.pool] || row.pool || '—') + ' · ' + props.t(row.priceSource === 'custom' || row.priceSource === 'override' ? 'settings.custom' : row.priceSource === 'reference' ? 'settings.reference' : row.priced ? 'settings.sourceOld' : 'settings.none'))),
