 ...
 * Billing groups stay separate. Display currency changes formatting only.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var LAYOUT_CSS = `
      ...
      .api-ledger-config { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; margin: 12px 0; }
      .api-ledger-config label { display: grid; gap: 4px; font-size: 12px; color: ${C.dim}; }
      .api-ledger-config input, .api-ledger-config select { width: 100%; min-width: 0; box-sizing: border-box; padding: 7px; border: 1px solid ${C.border}; border-radius: 6px; background: ${C.card}; color: ${C.text}; font: inherit; }
      .api-ledger-save { padding: 7px 12px; border: 1px solid ${C.border}; border-radius: 6px; cursor: pointer; color: ${C.text}; background: ${C.card}; }
      .api-ledger-config input:focus-visible { outline: 2px solid var(--ledger-accent); }
      @container api-ledger (max-width: 360px) { .api-ledger-config { grid-template-columns: 1fr; } }
    ...
    var MESSAGES = [
      ...
      ['settings.active', '当前已配置', 'Currently configured'],
      ['settings.inactive', '历史与已停用接口', 'Historical and inactive routes'],
      ['settings.history', '仅历史记录', 'Historical records'],
      ['settings.preferencesOnly', '仅账本配置', 'Ledger preferences only'],
      ['settings.remove', '删除账本配置', 'Delete ledger preferences'],
      ['settings.reset', '重置账本配置', 'Reset ledger preferences'],
      ['settings.removeNote', '清除该路由的名称、分组与价格设置；保留 DSH 模型配置和历史调用。', 'Clear this route’s name, group and pricing preferences. Provider settings and historical calls are retained.'],
      ['settings.title', '账户与计价', 'Accounts and pricing'],
      ['settings.note', '默认按路由分别统计。相同分组只合并展示，不代表共用余额。修改价格只影响之后的调用。', 'Routes are separate by default. A group combines reports, not balances. Price changes affect future calls only.'],
      ['settings.label', '显示名称', 'Display name'],
      ['settings.group', '账单分组（可留空）', 'Billing group (optional)'],
      ['settings.model', '模型 ID', 'Model ID'],
      ['settings.mode', '计价方式', 'Pricing mode'],
      ['settings.none', '未配置价格 / 仅记录用量', 'Usage only / no price configured'],
      ['settings.reference', '参考价估算（需确认适用）', 'Reference estimate (confirm applicability)'],
      ['settings.official', '官方价表估算（自动峰谷）', 'Official tariff estimate (peak-aware)'],
      ['settings.officialNote', '采用 2026-09-21 核验的官方 USD 价表，按请求开始时间及 2026 年中国节假日判断峰谷。价表随插件更新，并非实时拉取；未知模型或日历年份仅记录用量。', 'Uses the official USD tariff verified 2026-09-21, request start time and the 2026 Chinese holiday calendar. Updated with the plugin, not fetched live. Unknown models or calendar years are unpriced.'],
      ['report.suspectCurrency', '有 {count} 条记录疑似沿用了错误的 CNY 单价，历史金额可能偏低；原始记录已保留，请以供应商账单为准。', '{count} records may use legacy USD numbers tagged as CNY. Historical totals may be understated; original records are retained. Check the provider invoice.'],
      ['settings.peak', '高峰', 'Peak'],
      ['settings.offPeak', '非高峰', 'Off-peak'],
      ['settings.custom', '自定义单价估算', 'Custom price estimate'],
      ['settings.unit', '以下单价均为每百万 Token；缓存读写独立计价。', 'Prices per million tokens; cache reads and writes are priced separately.'],
      ['settings.save', '保存此路由', 'Save route'],
      ['settings.saved', '已保存，报告已更新。', 'Saved. Report updated.'],
      ['settings.currency', '计价币种', 'Billing currency'],
      ['settings.saving', '正在保存…', 'Saving…'],
      ['settings.error', '保存失败：配置可能已被修改，请重新打开此页后重试；检查价格和模型是否有效。', 'Save failed. Reopen this page if settings changed, and check model and prices.'],
      ['settings.unavailable', '配置文件无法读取或格式错误，已暂停计价；请修复配置后重试。', 'Settings cannot be read; pricing is paused until the file is repaired.'],
      ['settings.empty', '暂未发现 API 路由。请先在 DSH 中添加模型接口。', 'No API routes found. Add a model provider in DSH first.'],
      ['settings.cacheWrite', '缓存写入', 'Cache write'],
      ['settings.sourceOld', '历史估算', 'Historical estimate'],
      ['settings.referenceNote', '当前参考价为 DeepSeek 官方非高峰价格快照（2026-09-21，USD）；不自动判断峰谷和节假日，不代表实际扣费。', 'Snapshot: DeepSeek off-peak USD prices (2026-09-21). No peak-hour or holiday adjustment; not actual charges.'],
      ['report.subtitle', '按账单分组查看已记录的估算费用。', 'Estimated spend from recorded usage, by billing group.'],
      ...
      ['report.pools', '按账单分组分别计费', 'Spend by billing group'],
      ...
      ['report.apis', 'API 路由', 'API credentials'],
      ...
      ['sidebar.more', '+{count} 组', '+{count} groups'],
      ...
      ['page.subtitle', '按 API 凭据统计 token 与费用。不同账单分组分别列示，不合并计算。', 'Tokens and cost per API credential. Quota pools are listed separately and never summed.'],
    ...
    function createSource(ctx) {
      ...
      load.savePreference = function (args) {
        // This authenticated same-origin fallback also works on hosts whose RPC registration is unavailable.
        return fetch('/api-ledger/preferences', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(args) })
    ...
    function useReport(sessionId, range) {
      ...
      React.useEffect(function () {
        var alive = true
        function load(force) {
          if (source === null || force !== true && document.visibilityState === 'hidden') return
          var d = new Date()
          var payload = {
          ...
          source.load(payload).then(function (data) {
            ...
            if (data !== null && typeof data === 'object' && data.ok === true) {
              if (!data.usdRates || !data.today || !data.view || !Array.isArray(data.viewPools) || !Array.isArray(data.recent) || !data.settings) {
        ...
        var refresh = function () { load(true) }
        document.addEventListener('api-ledger:changed', refresh)
        return function () {
          alive = false
          clearInterval(timer)
          document.removeEventListener('visibilitychange', onVisible)
          document.removeEventListener('api-ledger:changed', refresh)
        }
    ...
    /** One billing group, including any calls whose price is unknown. */
    ...
    function AccountSettings(props) {
      ...
      var t = props.t
      return React.createElement('details', { className: 'api-ledger-disclosure' },
        React.createElement('summary', null, t('settings.title')),
        React.createElement('p', { style: S.muted }, t('settings.note')),
        saved[0] ? React.createElement('p', { role: 'status' }, t('settings.saved')) : null,
        settings.error ? React.createElement('p', { role: 'alert' }, t('settings.unavailable')) :
        !settings.routes.length ? React.createElement('p', null, t('settings.empty')) :
        React.createElement(React.Fragment, null,
          settings.routes.filter(function (route) { return route.active }).map(renderRoute),
          settings.routes.some(function (route) { return !route.active }) ? React.createElement('details', { className: 'api-ledger-disclosure' },
            React.createElement('summary', null, t('settings.inactive')),
            settings.routes.filter(function (route) { return !route.active }).map(renderRoute)) : null))
      function renderRoute(route) { return React.createElement(RouteSettings, { key: settings.revision + route.route, route: route, settings: settings, t: t, onSaved: function () { saved[1](true) } }) }
    }
    ...
    function RouteSettings(props) {
      ...
      var modelState = React.useState(route.models[0] || '')
      var model = modelState[0]
      var price = props.settings.pricing[route.route + '/' + model] || { mode: route.official ? 'official' : 'none' }
      var formState = React.useState({ label: route.label, group: route.group, mode: price.mode || 'custom',
        currency: price.currency || 'CNY', inputPerM: price.inputPerM == null ? '' : String(price.inputPerM), outputPerM: price.outputPerM == null ? '' : String(price.outputPerM),
        cacheReadPerM: String(price.cacheReadPerM || 0), cacheWritePerM: String(price.cacheWritePerM || 0) })
      var form = formState[0]
      var status = React.useState('')
      function set(key, value) { formState[1](Object.assign({}, form, { [key]: value })) }
      function selectModel(value) {
        modelState[1](value)
        var p = props.settings.pricing[route.route + '/' + value] || { mode: route.official ? 'official' : 'none' }
        formState[1](Object.assign({}, form, { mode: p.mode || 'custom', currency: p.currency || 'CNY', inputPerM: p.inputPerM == null ? '' : String(p.inputPerM), outputPerM: p.outputPerM == null ? '' : String(p.outputPerM), cacheReadPerM: String(p.cacheReadPerM || 0), cacheWritePerM: String(p.cacheWritePerM || 0) }))
      }
      function field(key, label, numeric) {
        return React.createElement('label', { key: key }, t(label), React.createElement('input', { value: form[key], type: numeric ? 'number' : 'text', min: numeric ? 0 : undefined, step: numeric ? 'any' : undefined,
          required: numeric, maxLength: numeric ? undefined : 160, onChange: function (e) { set(key, e.target.value) } }))
      }
      function remove() {
        status[1]('saving')
        source.load.savePreference({ token: props.settings.token, revision: props.settings.revision, route: route.route, action: 'remove' }).then(function () {
          props.onSaved()
          document.dispatchEvent(new Event('api-ledger:changed'))
        }).catch(function () { status[1]('error') })
      }
      function save(e) {
        e.preventDefault()
        status[1]('saving')
        var rates = { currency: form.currency }
        ;['inputPerM', 'outputPerM', 'cacheReadPerM', 'cacheWritePerM'].forEach(function (key) { rates[key] = form[key] === '' ? null : Number(form[key]) })
        source.load.savePreference({ token: props.settings.token, revision: props.settings.revision, route: route.route, label: form.label, group: form.group, model: model,
          mode: form.mode, rates: rates }).then(function () {
            status[1]('')
            props.onSaved()
            document.dispatchEvent(new Event('api-ledger:changed'))
          }).catch(function () { status[1]('error') })
      ...
      return React.createElement('details', { className: 'api-ledger-disclosure' },
        React.createElement('summary', null, route.label + ' · ' + route.route),
        React.createElement('p', { style: S.faintText }, t(route.active ? 'settings.active' : route.historical ? 'settings.history' : 'settings.preferencesOnly')),
        React.createElement('form', { onSubmit: save },
          React.createElement('div', { className: 'api-ledger-config' },
            field('label', 'settings.label'), field('group', 'settings.group'),
            React.createElement('label', null, t('settings.model'), React.createElement('input', { value: model, list: 'ledger-models-' + route.route, maxLength: 160, onChange: function (e) { selectModel(e.target.value) } }),
              React.createElement('datalist', { id: 'ledger-models-' + route.route }, route.models.map(function (m) { return React.createElement('option', { key: m, value: m }) }))),
            React.createElement('label', null, t('settings.mode'), React.createElement('select', { value: form.mode, disabled: !model, onChange: function (e) { set('mode', e.target.value) } },
              (route.official ? ['official', 'none', 'reference', 'custom'] : ['none', 'reference', 'custom']).map(function (mode) { return React.createElement('option', { key: mode, value: mode, disabled: (mode === 'reference' || mode === 'official') && props.settings.referenceModels.indexOf(model) < 0 }, t({ official: 'settings.official', none: 'settings.none', reference: 'settings.reference', custom: 'settings.custom' }[mode])) }))),
            form.mode === 'custom' ? React.createElement(React.Fragment, null,
              React.createElement('label', null, t('settings.currency'), React.createElement('select', { value: form.currency, onChange: function (e) { set('currency', e.target.value) } }, ['CNY', 'USD'].map(function (u) { return React.createElement('option', { key: u, value: u }, u) }))),
              field('inputPerM', 'col.input', true), field('outputPerM', 'col.output', true), field('cacheReadPerM', 'col.cacheRead', true), field('cacheWritePerM', 'settings.cacheWrite', true)) : null),
          form.mode === 'official' && props.settings.officialPrices && props.settings.officialPrices[model] ? React.createElement('p', { style: S.muted },
            t(props.settings.officialPrices[model].tariff === 'peak' ? 'settings.peak' : 'settings.offPeak') + ' · ' +
            ['inputPerM', 'outputPerM', 'cacheReadPerM'].map(function (key, i) { return t(['col.input', 'col.output', 'col.cacheRead'][i]) + ' ' + props.settings.officialPrices[model].rates[key] }).join(' · ') + ' USD / 1M tok',
            React.createElement('a', { href: 'https://api-docs.deepseek.com/quick_start/pricing/', target: '_blank', rel: 'noreferrer', style: { marginLeft: 8 } }, 'DeepSeek')) : null,
          form.mode === 'reference' && props.settings.referenceRates && props.settings.referenceRates[model] ? React.createElement('p', { style: S.muted },
            ['inputPerM', 'outputPerM', 'cacheReadPerM'].map(function (key, i) { return t(['col.input', 'col.output', 'col.cacheRead'][i]) + ' ' + props.settings.referenceRates[model][key] }).join(' · ') + ' USD / 1M tok',
            React.createElement('a', { href: 'https://api-docs.deepseek.com/quick_start/pricing/', target: '_blank', rel: 'noreferrer', style: { marginLeft: 8 } }, 'DeepSeek')) : null,
          React.createElement('p', { style: S.faintText }, t(form.mode === 'official' ? 'settings.officialNote' : form.mode === 'reference' ? 'settings.referenceNote' : form.mode === 'none' ? 'settings.none' : 'settings.unit')),
          React.createElement('button', { type: 'submit', className: 'api-ledger-save', disabled: status[0] === 'saving' }, t(status[0] === 'saving' ? 'settings.saving' : 'settings.save')),
          route.hasPreferences ? React.createElement('div', { style: { marginTop: 12 } }, React.createElement('p', { style: S.faintText }, t('settings.removeNote')), React.createElement('button', { type: 'button', className: 'api-ledger-save', disabled: status[0] === 'saving', onClick: remove }, t(route.active ? 'settings.reset' : 'settings.remove'))) : null,
          status[0] === 'error' ? React.createElement('p', { role: 'alert', className: 'api-ledger-warning' }, t('settings.error')) : null))
    }
    ...
    function RecentCalls(props) {
      ...
      var rows = props.rows.map(function (row, i) {
          ...
          return React.createElement('li', { key: i },
            React.createElement('div', null, row.model || '—', React.createElement('small', null, when + ' · ' + (row.route || '—') + ' · ' + (props.labels[row.pool] || row.pool || '—') + ' · ' + props.t(row.priceSource === 'official' ? 'settings.official' : row.priceSource === 'custom' || row.priceSource === 'override' ? 'settings.custom' : row.priceSource === 'reference' ? 'settings.reference' : row.priced ? 'settings.sourceOld' : 'settings.none'))),
    ...
    function ReportBody(props) {
      ...
      return React.createElement('div', { style: S.page },
        ...
        React.createElement(React.Fragment, null,
          !props.session ? React.createElement(AccountSettings, { settings: data.settings, t: t }) : null,
          ...
          !props.session ? React.createElement(RecentCalls, { rows: data.recent || [], labels: labels, t: t }) : null,
          num(data.suspectCurrencyCount) > 0 ? React.createElement('p', { className: 'api-ledger-warning', role: 'alert' }, t('report.suspectCurrency', { count: data.suspectCurrencyCount })) : null,
        ...
        return React.createElement('button', {
          ...
          !first || error ? React.createElement('span', null, status) :
            React.createElement('span', { className: 'api-ledger-status-pool' },
              React.createElement('span', null, data.poolLabels[first.pool] || first.pool),
            ...
            pools.length === 0 || error ? React.createElement('span', null, status) :
              pools.map(function (pool) {
                return React.createElement('span', { key: pool.pool, className: 'api-ledger-status-pool' },
                  React.createElement('span', { style: { color: POOL_COLOR[pool.pool] || C.dim } }, data.poolLabels[pool.pool] || pool.pool),
