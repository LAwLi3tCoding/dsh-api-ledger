 ...
 * ## What it answers
 *
 * "Which key spent how many tokens, and what did that cost?" — and, separately,
 * "how much of my DeepSeek balance is left". Those are different questions and
 * this plugin keeps their numbers apart (see `pools` in `pricing.js`).
 *
...
import { DEFAULT_POOLS, POOL_LABELS, USD_RATES, ratesFor, priceUsage, poolOf } from './pricing.js'
...
/** Read the user's override file. A broken file degrades to "no overrides". */
...
export function apply(ctx) {
  ...
  /** Overrides are re-read lazily so editing the file needs no restart. */
  let configCache = { at: 0, value: null }
  function overrides() {
    const now = Date.now()
    if (configCache.value !== null && now - configCache.at < 5_000) return configCache.value
    const cfg = readConfig(configFile)
    const value = {
      pricing: cfg.pricing !== null && typeof cfg.pricing === 'object' ? cfg.pricing : {},
      pools: cfg.pools !== null && typeof cfg.pools === 'object' ? cfg.pools : DEFAULT_POOLS,
    }
  ...
  ctx.effect(() => ctx.on('llm/stream', (options, next) => {
    ...
    // Peak/off-peak classification must use the REQUEST time, not completion:
    // a stream crossing the boundary hour would otherwise be filed under the
    // wrong tariff.
  ...
  function report(args = {}) {
    ...
    const cfg = overrides()
    const sessionRecords = sessionId === null ? [] : ledger.recordsForSession(sessionId)
    const scoped = sessionId === null ? ledger.records : sessionRecords
    ...
    const today = ledger.foldRecords(ledger.records.filter((rec) => todayStart > 0 && Number(rec.time) >= todayStart))
    ...
    return {
      ...
      yesterday: Number(a.yesterdayStart) > 0 && Number(a.yesterdayStart) < todayStart
        ? ledger.foldRecords(ledger.records.filter((rec) => Number(rec.time) >= Number(a.yesterdayStart) && Number(rec.time) < todayStart)) : null,
      recent: viewRecords.filter((rec) => rec.usageMissing !== true).slice().sort((a, b) => Number(b.time) - Number(a.time)).slice(0, 15).map((rec) => ({
        ...
        priced: rec.priced === true, usd: rec.usd,
      })),
      view,
    ...
    child.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          send(report(await readJsonBody(req)))
        } catch (error) {
