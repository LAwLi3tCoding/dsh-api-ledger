...
import { USD_RATES, REFERENCE_MODELS, REFERENCE_RATES, REFERENCE_INFO, officialRates, suspectCurrency, ratesFor, priceUsage, poolOf } from './pricing.js'
...
export function apply(ctx) {
  ...
  async function identities(force = false) {
    const now = Date.now()
    if (!force && identityCache.value !== null && now - identityCache.at < IDENTITY_TTL_MS) return identityCache.value
    const value = await resolveIdentities({ llm: ctx.get('llm'), settings: ctx.get('settings'), environment: ctx.get('launchEnvironment') })
  ...
  function account(usage, options, startedAt) {
    ...
    const cfg = overrides()
    const matched = cfg.error ? null : ratesFor(route, model, cfg.pricing, { official: identityCache.value?.[route]?.official === true, time: startedAt })
    ...
    const record = {
      ...
      tariff: matched?.tariff || null,
      priceCheckedAt: matched?.checkedAt || null,
    }
  ...
  async function report(args = {}) {
    ...
    const routes = knownRoutes.map(route => ({
      official: directory[route]?.official === true,
    ...
    return {
      ...
      suspectCurrencyCount: viewRecords.filter(suspectCurrency).length,
      home,
      ...
      poolLabels: labels,
      settings: { token: settingsToken, revision, routes, pricing: cfg.pricing, referenceModels: REFERENCE_MODELS, referenceRates: REFERENCE_RATES, referenceInfo: REFERENCE_INFO, officialPrices: Object.fromEntries(REFERENCE_MODELS.map(model => [model, officialRates(model, Date.now())])), error: cfg.error || null },
      usdRates: USD_RATES,
      today,
      ...
      recent: viewRecords.filter((rec) => rec.usageMissing !== true).slice().sort((a, b) => Number(b.time) - Number(a.time)).slice(0, 15).map((rec) => ({
        ...
        priced: rec.priced === true, usd: rec.usd, priceSource: rec.priceSource, tariff: rec.tariff, suspectCurrency: suspectCurrency(rec), currency: rec.currency, base: rec.base,
      })),
      view,
    ...
    try {
      const dispose = child.connection.rpc.handle('/api-ledger', {
        ...
        savePreference: async (args) => {
          ...
          return savePreference(configFile, args, data.settings.routes.map(r => r.route), data.settings.routes.filter(r => r.official).map(r => r.route))
        },
      })
    ...
    child.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const data = await report()
          send(200, savePreference(configFile, args, data.settings.routes.map(r => r.route), data.settings.routes.filter(r => r.official).map(r => r.route)))
