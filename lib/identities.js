 ...
 */
export async function resolveIdentities({ llm, settings, environment } = {}) {
  ...
  for (const entry of directory) {
    ...
    let baseURL = typeof cfg.baseURL === 'string' ? cfg.baseURL : null
    if (baseURL === null && entry.settingsNs === 'llm-deepseek') {
      try { baseURL = (environment ? environment.get('DEEPSEEK_BASE_URL')?.value : process.env.DEEPSEEK_BASE_URL) ?? 'https://api.deepseek.com' } catch { baseURL = null }
    }
    let official = false
    try {
      const url = new URL(baseURL)
      official = url.origin === 'https://api.deepseek.com' && !url.username && !url.password
        && !url.search && !url.hash && ['', '/', '/v1', '/v1/', '/anthropic', '/anthropic/'].includes(url.pathname)
    } catch {}
    out[provider] = {
      official,
      provider,
