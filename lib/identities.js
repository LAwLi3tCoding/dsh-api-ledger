...
/**
 * The identity a record is filed under.
 *
 * Preference order, and why:
 *   1. The credential reference — stable across route renames, and the thing
 *      the user actually thinks in ("which key").
 *   2. The route name — always available, and still separates two keys.
 *   3. `unknown` — a call captured before the directory existed.
 *
 * @param {string} route
 * @param {Record<string, object>} identities
 * @returns {{ id: string, label: string, keyRef: string|null }}
 */
export function identityOf(route, identities) {
  ...
  return {
    id: entry.keyRef !== null ? entry.keyRef : route,
    label: entry.keyRef !== null ? entry.keyRef : entry.label,
    keyRef: entry.keyRef,
  }
}
